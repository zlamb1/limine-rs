use core::{ffi::CStr, ffi::c_char, ptr::null};

use crate::utility::as_cstr;

pub const REQUEST_ID: [u64; 2] = [0xf55038d8e2a1202f, 0x279426fcf5f59740];

#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
#[repr(C)]
pub struct Request {
    base: crate::Request,
    response: *const Response,
}

impl Request {
    pub const fn new() -> Self {
        Self {
            base: crate::Request::new(REQUEST_ID[0], REQUEST_ID[1]),
            response: null(),
        }
    }

    pub const fn base(&self) -> &crate::Request {
        &self.base
    }

    pub const fn response_ptr(&self) -> *const Response {
        self.response
    }

    pub const fn response(&self) -> Option<&'static Response> {
        unsafe { self.response.as_ref() }
    }
}

impl Default for Request {
    fn default() -> Self {
        Self::new()
    }
}

unsafe impl Send for Request {}
unsafe impl Sync for Request {}

#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
#[repr(C)]
pub struct Response {
    rev: u64,
    name: *const c_char,
    version: *const c_char,
}

impl Response {
    pub const fn revision(&self) -> u64 {
        self.rev
    }

    pub const fn name_ptr(&self) -> *const c_char {
        self.name
    }

    pub const fn name(&self) -> Option<&CStr> {
        unsafe { as_cstr(self.name) }
    }

    pub const fn version_ptr(&self) -> *const c_char {
        self.version
    }

    pub const fn version(&self) -> Option<&CStr> {
        unsafe { as_cstr(self.version) }
    }
}

unsafe impl Send for Response {}
unsafe impl Sync for Response {}
